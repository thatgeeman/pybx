...
Docs: https://thatgeeman.github.io/pybx/basics.html.md"""

# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/01_basics.ipynb.

# %% auto #0
__all__ = ['COORD_TYPES', 'LABEL_TYPES', 'ITER_TYPE_JSON', 'ITER_TYPES', 'ITER_TYPES_TUPLE', 'ITER_TYPES_EXTRA',
           'ITER_TYPE_HASHED', 'ALL_ITER_TYPES', 'ALL_TYPES', 'BX_TYPE', 'check_format_types', 'check_length_types',
           'check_max_voc', 'perform_box_checks', 'parse_list', 'parse_dict', 'parse_json', 'Bx', 'BaseBx', 'bbx',
           'jbx', 'dbx', 'parse_basebx', 'infer_box_dtype', 'parse_to_bbx', 'MultiBx', 'mbx', 'get_bx', 'stack_bxs',
           'add_bxs', 'stack_bxs_inplace']

# %% ../nbs/01_basics.ipynb #6ec4ba3d
import warnings
import inspect
import json
...
def check_format_types(b: list, verbose=False):
    ...
    Returns:
        bool
    """
    result = len(b) in (4, 5) and all(
        isinstance(x, COORD_TYPES) for x in b[:4]
    )
    if len(b) == 5:
        result = result and isinstance(b[4], LABEL_TYPES)
    if verbose:
        verbose_result = "Passed" if result else "Failed"
        print(f"{verbose_result} `check_format_types`.")
    return result

# %% ../nbs/01_basics.ipynb #437ea81d
def check_length_types(b: list, verbose=False):
    ...
    """
    b_label = True
    # exactly four coordinates and, optionally, one label
    b_len = len(b) in (4, 5)
    ...
    if len(b) == 5:
        b_label = isinstance(b[4], LABEL_TYPES)
...
def parse_dict(b: dict, **kwargs):
    ...
    Returns:
        ...
        str: Bounding box label
    """
    missing = [key for key in voc_keys[:4] if key not in b]
    if missing:
        raise ValueError(f"Missing coordinate keys {missing} in bounding box {b}")

    values = [b[key] for key in voc_keys[:4]]
    label_key = next((key for key in label_keys if key in b), None)
    if label_key is not None:
        values.append(b[label_key])
    return parse_list(values, **kwargs)

# %% ../nbs/01_basics.ipynb #f745ed4a
...
class Bx:
    ...
    def __init__(self, coords, label: list = None, verbose=False, no_check=False):
        ...
        label = label if label is not None else parsed_label
        label = label if is_listy(label) else [label]
        if len(label) != 1:
            raise ValueError(f"BaseBx requires exactly one label, got {len(label)}.")
...
# %% ../nbs/01_basics.ipynb #0c8f5d93
def jbx(coords=None, label=None, no_check=False):
    """Alias of the `BaseBx` class using json strings."""
    coords, parsed_label = parse_json(coords, no_check=no_check)
    # make label a list of single item
    label = label if label is not None else parsed_label
    label = label if is_listy(label) else [label]
...
# %% ../nbs/01_basics.ipynb #e877e73c
def dbx(coords=None, label=None, no_check=False):
    """Alias of the `BaseBx` class using dict."""
    coords, parsed_label = parse_dict(coords, no_check=no_check)
    # make label a list of single item
    label = label if label is not None else parsed_label
    label = label if is_listy(label) else [label]
...
def infer_box_dtype(b: ALL_ITER_TYPES, **kwargs):
    if isinstance(b, str):
        ...
        return "list"
    elif isinstance(b, (list, L, tuple)):
        return "list"
    else:
...
def _as_box_data(boxes):
    """Normalize a Bx object to coordinate and label lists for stacking."""
    if isinstance(boxes, BaseBx):
        parsed = [boxes]
    else:
        parsed = [boxes[idx] for idx in range(len(boxes.coords))]
    return [box._coords for box in parsed], [box.label[0] for box in parsed]

# %% ../nbs/01_basics.ipynb #c6a596bd
def mbx(coords=None, label=None, no_check=False):
    """Alias of the `MultiBx` class."""
    return MultiBx(coords, label, no_check=no_check)

# %% ../nbs/01_basics.ipynb #c95f363c
def get_bx(coords, label=None, no_check=False):
    ...
    if isinstance(coords, str):
        return jbx(coords, label, no_check=no_check)
    # process list
    ...
    if isinstance(coords, dict):
        return dbx(coords, label, no_check=no_check)
    # process tuple
...
def __add__(self: BaseBx, other):
    ...
    self_coords, self_labels = _as_box_data(self)
    other_coords, other_labels = _as_box_data(other)
    coords = self_coords + other_coords
    label = self_labels + other_labels
