...
class OpsTestCase(unittest.TestCase):
    ...
    def test_make_array(self):
        with open(params["annots_iou_file"]) as f:
            annots = json.load(f)
        array, label = ops.dict_array(annots[0])
    ...
    def test_named_idx(self):
        with open(params["annots_iou_file"]) as f:
            annots = json.load(f)
        array, label = ops.dict_array(annots[1])
    ...
    def test_intersection_box(self):
        with open(params["annots_iou_file"]) as f:
            annots = json.load(f)
        a0, _ = ops.dict_array(annots[0])
        a1, _ = ops.dict_array(annots[1])
        a2, _ = ops.dict_array(annots[2])
    ...
    def test_intersection_box_noint(self):
        ...
        with open(params["annots_rand_file"]) as f:
            annots1 = json.load(f)
        a1, _ = ops.dict_array(annots0[0])
        a2, _ = ops.dict_array(annots1[1])
